...
# DNS서버에서 페이지를 불러오는 방법
  - DNS 서버는 사용자가 웹 브라우저에 입력한 도메인 이름을 해당하는 IP 주소로 변환하여 사용자가 요청한 페이지를
    ...
    ### 동작 과정
        1. 도메인 입력
          - 사용자가 브라우저에 www.example.com과 같은 URL을 입력한다
          - 브라우저는 이 URL의 도메인 이름을 사용하여 IP 주소를 알아내야한다
        2. 로컬 DNS 캐시 확인
          - 먼저, 브라우저와 운영체제는 로컬캐시(Local Cache)에 해당 도메인의 IP 주소가 있는지 확인한다
            * 캐시에 IP주소가 있다면 바로 사용하고, DNS 서버로 요청하지않는다
            * 캐시 확인위치: 브라우저 캐시, 운영체제(OS)캐시
        3. 리졸버(Resolver) DNS 서버에 요청
          - 로컬 캐시에 없을 경우, 요청은 리졸버 DNS 서버로 전송 된다
          - 리졸버는 도메인 이름에 대한 IP주소를 찾아주기 위해 다른 DNS 서버와 협력한다.    