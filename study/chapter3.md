...
# 자바스크립트의 메모리 구조
  ...
  대표적인 예시로는 구글의 V8엔진이 있다. 파이어폭스의 SpiderMonkey, 사파리의 Webkit 등이 있지만, 크롬에서 사용하는 V8이 가장 대중적으로 알려져 있다.
  ...
  **Call Stack**   
  ...
  작업을 수행하는 공간이다. 특정 함수를 실행하게 되면, 해당 함수는 콜 스택의 가장 상단에 위치하게되며 함수 실행이 끝나게 되면,  
  해당 함수는 콜 스택에서 제거되게 된다.  
